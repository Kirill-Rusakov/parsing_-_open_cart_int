...
try {
    $db_data = new PDO('mysql:host=89.111.154.213;dbname=data_db;charset=utf8', 'main_admin', 'StrongP@ssw0rd');
...
function addOrUpdateProductsInOpenCart($db_data, $db_opencart, $sourceTable, $categoryId) {
    ...
    while ($product = $products->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (strpos(mb_strtolower(trim($product['status'])), 'снят с произв-ва') !== false) {
            continue;
        }

        // Разбираем JSON из колонки image
        $images = json_decode($product['image'], true);
        if (json_last_error() !== JSON_ERROR_NONE || empty($images)) {
            continue; // Пропускаем товар, если JSON некорректный или пустой
        }

        $main_image = htmlspecialchars($images[0]); // Первое изображение как основное
        $additional_images = array_slice($images, 1); // Остальные изображения
        ...
        if ($existingProduct) {
            ...
            $stmt = $db_opencart->prepare("UPDATE oc_product SET image = :image, price = :price, date_modified = NOW() WHERE product_id = :product_id");
            $stmt->execute([
                'image' => $main_image,
