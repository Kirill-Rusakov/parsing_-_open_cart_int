...
function parseCatalog($urlArray, $db, $tableName) {
    ...
    foreach ($urlArray as $url) {
        ...
        if ($html === false) {
            error_log("Не удалось получить содержимое страницы: $html");
            continue;
        }
        ...
        foreach ($catalog as $item) {
            $link = $item->find(".img_box .for_img a")[0]->attr("href");
            ...
            if ($html2 === false) {
                error_log("Не удалось получить содержимое страницы: $html2");
                continue;
            }
            $document->loadHtml($html2);

            $images = [];
            foreach ($document->find(".card_row .card_left .SliderCard .slide a img") as $imgElement) 
            {
                $images[] = "https://nkamin.ru" . $imgElement->attr("src");
            }
